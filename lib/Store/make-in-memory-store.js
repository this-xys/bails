...
const waChatKey = (pin) => ({ key: (c) => (pin ? c.pinned ? "1" : "0" : "") + (c.archived ? "0" : "1") + (c.conversationTimestamp ? c.conversationTimestamp.toString(16).padStart(8, "0") : "") + c.id, compare: (k1, k2) => k2.localeCompare(k1) });
const waMessageID = (m) => m.key.id || "";
const waLabelAssociationKey = { key: (la) => la.type === LabelAssociationType.Chat ? la.chatId + la.labelId : la.chatId + la.messageId + la.labelId, compare: (k1, k2) => k2.localeCompare(k1) };
...
const makeInMemoryStore = (config = {}) => {
  ...
  const bind = (ev) => {
    ...
    ev.on("contacts.upsert", (contacts2) => {
      contactsUpsert(contacts2);
    });
      ...
      for (const update of updates) {
        ...
        if (contact) {
          if (update.imgUrl === "changed") {
            contact.imgUrl = socket ? await (socket === null || socket === void 0 ? void 0 : socket.profilePictureUrl(contact.id)) : void 0;
        ...
        switch (action) {
          ...
          case "remove":
            ...
            for (const p of participants) {
              if (p.id) removeSet.add(p.id);
              if (p.phoneNumber) removeSet.add(p.phoneNumber);
            }
            metadata.participants = metadata.participants.reduce((acc, p) => {
              if (!removeSet.has(p.id) && !removeSet.has(p.phoneNumber)) acc.push(p);
              return acc;
            }, []);
            break;
        }
  ...
  return { chats, contacts, messages, groupMetadata, state, presences, labels, labelAssociations, bind, loadMessages: async (jid, count, cursor) => {
    const list = assertMessageList(jid);
    const mode = !cursor || "before" in cursor ? "before" : "after";
    const cursorKey = !!cursor ? "before" in cursor ? cursor.before : cursor.after : void 0;
    const cursorValue = cursorKey ? list.get(cursorKey.id) : void 0;
    let messages2;
    if (list && mode === "before" && (!cursorKey || cursorValue)) {
      if (cursorValue) {
        const msgIdx = list.array.findIndex((m) => m.key.id === (cursorKey === null || cursorKey === void 0 ? void 0 : cursorKey.id));
        messages2 = list.array.slice(0, msgIdx);
      } else {
        messages2 = list.array;
      }
      const diff = count - messages2.length;
      if (diff < 0) {
        messages2 = messages2.slice(-count);
      }
    } else {
      messages2 = [];
    }
  ...
  }, toJSON, fromJSON, writeToFile: (path) => {
