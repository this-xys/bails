...
const createSignalIdentity = (wid, accountSignatureKey) => {
  return { identifier: { name: wid, deviceId: 0 }, identifierKey: generateSignalPubKey(accountSignatureKey) };
};
...
const xmppSignedPreKey = (key) => ({ tag: "skey", attrs: {}, content: [{ tag: "id", attrs: {}, content: encodeBigEndian(key.keyId, 3) }, { tag: "value", attrs: {}, content: key.keyPair.public }, { tag: "signature", attrs: {}, content: key.signature }] });
const xmppPreKey = (pair, id) => ({ tag: "key", attrs: {}, content: [{ tag: "id", attrs: {}, content: encodeBigEndian(id, 3) }, { tag: "value", attrs: {}, content: pair.public }] });
...
const extractE2ESessionFromRetryReceipt = (receipt) => {
  ...
  return { registrationId, identityKey: generateSignalPubKey(identity), signedPreKey: { keyId: signedKeyId, publicKey: generateSignalPubKey(signedPubKey), signature: signedSig }, preKey };
};
const parseAndInjectE2ESessions = async (node, repository) => {
  const extractKey = (key) => key ? { keyId: getBinaryNodeChildUInt(key, "id", 3), publicKey: generateSignalPubKey(getBinaryNodeChildBuffer(key, "value")), signature: getBinaryNodeChildBuffer(key, "signature") } : void 0;
  const nodes = getBinaryNodeChildren(getBinaryNodeChild(node, "list"), "user");
  for (const node2 of nodes) {
    assertNodeErrorFree(node2);
  }
  const chunkSize = 100;
  ...
  for (const nodesChunk of chunks) {
    for (const node2 of nodesChunk) {
      ...
      const key = getBinaryNodeChild(node2, "key");
      const identity = getBinaryNodeChildBuffer(node2, "identity");
      const jid = node2.attrs.jid;
      const registrationId = getBinaryNodeChildUInt(node2, "registration", 4);
      await repository.injectE2ESession({ jid, session: { registrationId, identityKey: generateSignalPubKey(identity), signedPreKey: extractKey(signedKey), preKey: extractKey(key) } });
    }
  }
};
const extractDeviceJids = (result, myJid, myLid, excludeZeroDevices) => {
  ...
  for (const userResult of result) {
    ...
    for (const { id: device, keyIndex, isHosted } of deviceList) {
      if ((!excludeZeroDevices || device !== 0) && (myUser !== user && myLid !== user || myDevice !== device) && (device === 0 || !!keyIndex)) {
        if (isHosted) {
...
const getNextPreKeys = async ({ creds, keys }, count) => {
  ...
  const update = { nextPreKeyId: Math.max(lastPreKeyId + 1, creds.nextPreKeyId), firstUnuploadedPreKeyId: Math.max(creds.firstUnuploadedPreKeyId, lastPreKeyId + 1) };
...
const getNextPreKeysNode = async (state, count) => {
  ...
  const node = { tag: "iq", attrs: { xmlns: "encrypt", type: "set", to: S_WHATSAPP_NET }, content: [{ tag: "registration", attrs: {}, content: encodeBigEndian(creds.registrationId) }, { tag: "type", attrs: {}, content: KEY_BUNDLE_TYPE }, { tag: "identity", attrs: {}, content: creds.signedIdentityKey.public }, { tag: "list", attrs: {}, content: Object.keys(preKeys).map((k) => xmppPreKey(preKeys[+k], +k)) }, xmppSignedPreKey(creds.signedPreKey)] };
  return { update, node };
};
export {
