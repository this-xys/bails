...
const shouldIncludeReportingToken = (message) => !message.reactionMessage && !message.encReactionMessage && !message.encEventResponseMessage && !message.pollUpdateMessage;
const generateMsgSecretKey = (modificationType, origMsgId, origMsgSender, modificationSender, origMsgSecret) => {
  const useCaseSecret = Buffer.concat([Buffer.from(origMsgId, "utf8"), Buffer.from(origMsgSender, "utf8"), Buffer.from(modificationSender, "utf8"), Buffer.from(modificationType, "utf8")]);
...
const getMessageReportingToken = async (msgProtobuf, message, key) => {
  ...
  return { tag: "reporting", attrs: {}, content: [{ tag: "reporting_token", attrs: { v: "2" }, content: reportingToken }] };
};
export {
