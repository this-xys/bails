...
const parseCatalogNode = (node) => {
  ...
  return { products, nextPageCursor: paging ? getBinaryNodeChildString(paging, "after") : void 0 };
};
const parseCollectionsNode = (node) => {
  ...
  const collections = getBinaryNodeChildren(collectionsNode, "collection").map((collectionNode) => {
    ...
    return { id, name, products, status: parseStatusInfo(collectionNode) };
  });
  return { collections };
};
const parseOrderDetailsNode = (node) => {
  ...
  const orderDetails = { price: { total: +getBinaryNodeChildString(priceNode, "total"), currency: getBinaryNodeChildString(priceNode, "currency") }, products };
  return orderDetails;
};
const toProductNode = (productId, product) => {
  ...
  if (product.images.length) {
    content.push({ tag: "media", attrs: {}, content: product.images.map((img) => {
      if (!("url" in img)) {
  ...
  if ("originCountryCode" in product) {
    if (typeof product.originCountryCode === "undefined") {
      ...
      content.push({ tag: "compliance_info", attrs: {}, content: [{ tag: "country_code_origin", attrs: {}, content: Buffer.from(product.originCountryCode) }] });
    }
  }
  if (typeof product.isHidden !== "undefined") {
    attrs["is_hidden"] = product.isHidden.toString();
  }
  const node = { tag: "product", attrs, content };
  return node;
};
const parseProductNode = (productNode) => {
  ...
  const product = { id, imageUrls: parseImageUrls(mediaNode), reviewStatus: { whatsapp: getBinaryNodeChildString(statusInfoNode, "status") }, availability: "in stock", name: getBinaryNodeChildString(productNode, "name"), retailerId: getBinaryNodeChildString(productNode, "retailer_id"), url: getBinaryNodeChildString(productNode, "url"), description: getBinaryNodeChildString(productNode, "description"), price: +getBinaryNodeChildString(productNode, "price"), currency: getBinaryNodeChildString(productNode, "currency"), isHidden };
  return product;
};
async function uploadingNecessaryImagesOfProduct(product, waUploadToServer, timeoutMs = 3e4) {
  product = { ...product, images: product.images ? await uploadingNecessaryImages(product.images, waUploadToServer, timeoutMs) : product.images };
  return product;
}
const uploadingNecessaryImages = async (images, waUploadToServer, timeoutMs = 3e4) => {
  const results = await Promise.all(images.map(async (img) => {
    ...
    await Promise.race([new Promise((resolve) => encFileWriteStream.end(resolve)), writeStreamError]);
    const sha = hasher.digest("base64");
    const { directPath } = await waUploadToServer(filePath, { mediaType: "product-catalog-image", fileEncSha256B64: sha, timeoutMs });
    await fs.unlink(filePath).catch((err) => logger.warn({ err, filePath }, "Fix (bug 57): error deleting temp file"));
