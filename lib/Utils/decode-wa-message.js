...
const DECRYPTION_RETRY_CONFIG = { maxRetries: 3, baseDelayMs: 100, sessionRecordErrors: ["No session record", "SessionError: No session record"] };
const NACK_REASONS = { SenderReachoutTimelocked: 463, ParsingError: 487, UnrecognizedStanza: 488, UnrecognizedStanzaClass: 489, UnrecognizedStanzaType: 490, InvalidProtobuf: 491, InvalidHostedCompanionStanza: 493, MissingMessageSecret: 495, SignalErrorOldCounter: 496, MessageDeletedOnPeer: 499, UnhandledError: 500, UnsupportedAdminRevoke: 550, UnsupportedLIDGroup: 551, DBOperationFailed: 552 };
const SERVER_ERROR_CODES = { MessageAccountRestriction: "463", SmaxInvalid: "479" };
...
function decodeMessageNode(stanza, meId, meLid) {
  ...
  const key = { remoteJid: chatId, remoteJidAlt: !isJidGroup(chatId) ? addressingContext.senderAlt : void 0, remoteJidUsername: !isJidGroup(chatId) ? stanza.attrs.peer_recipient_username || stanza.attrs.recipient_username : void 0, fromMe, id: msgId, participant, participantAlt: isJidGroup(chatId) ? addressingContext.senderAlt : void 0, participantUsername: stanza.attrs.participant ? stanza.attrs.participant_username : void 0, addressingMode: addressingContext.addressingMode, ...msgType === "newsletter" && stanza.attrs.server_id ? { server_id: stanza.attrs.server_id } : {} };
  const fullMessage = { key, category: stanza.attrs.category, messageTimestamp: +stanza.attrs.t, pushName: pushname, broadcast: isJidBroadcast(from) };
  if (key.fromMe) {
...
const decryptMessageNode = (stanza, meId, meLid, repository, logger) => {
  ...
  return { fullMessage, category: stanza.attrs.category, author, async decrypt() {
    let decryptables = 0;
    if (Array.isArray(stanza.content)) {
      for (const { tag, attrs, content } of stanza.content) {
        ...
        try {
          ...
          const errorContext = { key: fullMessage.key, err, messageType: tag === "plaintext" ? "plaintext" : attrs.type, sender, author, isSessionRecordError: isSessionRecordError(err) };
