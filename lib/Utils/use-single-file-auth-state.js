...
const useSingleFileAuthState = async (fileName) => {
  const cache = new LRUCache({ max: 2e4, ttl: 1e3 * DEFAULT_CACHE_TTLS.SIGNAL_STORE, updateAgeOnGet: false, updateAgeOnHas: false, ttlAutopurge: true });
  ...
  return { state: { creds, keys: { get: (type, ids) => {
    const data = {};
    for (const id of ids) {
      const keyName = type + id;
      let value = cache.get(keyName);
      if (value === void 0 && fileData[keyName] !== void 0) {
