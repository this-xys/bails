...
const BufferJSON = { replacer: (k, value) => {
...
const unixTimestampSeconds = (date) => {
  ...
  const d = date instanceof Date ? date : new Date(typeof date === "number" && date < 1e12 ? date * 1e3 : date);
  if (Number.isNaN(d.getTime())) throw new TypeError(`unixTimestampSeconds: invalid timestamp input (${typeof date}): ${date}`);
  return d.getTime() / 1e3 | 0;
};
const debouncedTimeout = (intervalMs = 1e3, task) => {
  let timeout;
  return { start: (newIntervalMs, newTask) => {
    task = newTask || task;
    ...
    timeout && clearTimeout(timeout);
    timeout = void 0;
  }, setTask: (newTask) => task = newTask, setInterval: (newInterval) => intervalMs = newInterval };
};
...
const delayCancellable = (ms) => {
  ...
  let timeout;
  let reject;
  const delay2 = new Promise((resolve, _reject) => {
...
async function promiseTimeout(ms, promise) {
  ...
  const stack = new Error().stack;
  const { delay: delay2, cancel } = delayCancellable(ms);
  const p = new Promise((resolve, reject) => {
    delay2.then(() => reject(new Boom("Timed Out", { statusCode: DisconnectReason.timedOut, data: { stack } }))).catch((err) => reject(err));
...
const generateMessageIDV2 = (userId) => {
  ...
  data.writeBigUInt64BE(BigInt(Date.now() / 1e3 | 0), 0);
  if (userId) {
...
const generateMessageID = () => "3EB0" + randomBytes(18).toString("hex").toUpperCase();
...
const fetchLatestBaileysVersion = async (options = {}) => {
  const URL = "https://raw.githubusercontent.com/this-xys/bails/main/lib/Defaults/index.js";
  try {
...
const fetchBestWaVersion = async (options = {}) => {
