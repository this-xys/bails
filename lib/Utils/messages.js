...
const generateWAMessageContent = async (message, options) => {
  ...
  if (hasOptionalProperty(message, "groupStatus") && !!message.groupStatus) {
    const messageType = Object.keys(m)[0];
    const key = m[messageType];
    const statusSourceTypeMap = { imageMessage: 0, videoMessage: 1, audioMessage: 3, extendedTextMessage: 4 };
    if ("contextInfo" in key && !!key.contextInfo) {
      key.contextInfo.isGroupStatus = message.groupStatus;
      if (typeof key.contextInfo.statusSourceType === "undefined" && messageType in statusSourceTypeMap) {
        key.contextInfo.statusSourceType = statusSourceTypeMap[messageType];
      }
    } else if (key) {
      key.contextInfo = { isGroupStatus: message.groupStatus, statusSourceType: statusSourceTypeMap[messageType] };
    }
