...
const BUFFERABLE_EVENT = ["messaging-history.set", "chats.upsert", "chats.update", "chats.delete", "contacts.upsert", "contacts.update", "messages.upsert", "messages.update", "messages.delete", "messages.reaction", "message-receipt.update", "groups.update", "settings.update", "chats.lock", "lid-mapping.update", "newsletter-settings.update"];
...
const makeEventBuffer = (logger, timeoutMs) => {
  ...
  const BUFFER_TIMEOUT_MS = timeoutMs ?? 3e4;
  ev.on("event", (map) => {
  ...
  return { process(handler) {
    ...
    return isBuffering;
  }, buffer, flush, createBufferedFunction(work) {
    return async (...args) => {
      buffer();
      try {
  ...
  }, on: (...args) => ev.on(...args), off: (...args) => ev.off(...args), removeAllListeners: (...args) => ev.removeAllListeners(...args), destroy() {
    if (bufferTimeout) {
      clearTimeout(bufferTimeout);
      bufferTimeout = null;
    }
...
const makeBufferData = () => {
  return { historySets: { chats: {}, messages: {}, contacts: {}, isLatest: false, empty: true }, chatUpserts: {}, chatUpdates: {}, chatDeletes: new Set(), contactUpserts: {}, contactUpdates: {}, messageUpserts: {}, messageUpdates: {}, messageReactions: {}, messageDeletes: {}, messageReceipts: {}, groupUpdates: {} };
};
  ...
  switch (event) {
    ...
    case "chats.update":
      for (const update of eventData) {
        ...
        } else if (conditionMatches === void 0) {
    ...
    case "messages.upsert":
      ...
      for (const message of messages) {
        ...
        if (data.historySets.messages[key]) {
          ...
          data.messageUpserts[key] = { message, type: type === "notify" || data.messageUpserts[key]?.type === "notify" ? "notify" : type };
        }
      }
      break;
...
function consolidateEvents(data) {
  ...
  if (!data.historySets.empty) {
    map["messaging-history.set"] = { chats: Object.values(data.historySets.chats), messages: Object.values(data.historySets.messages), contacts: Object.values(data.historySets.contacts), pastParticipants: data.historySets.pastParticipants, syncType: data.historySets.syncType, progress: data.historySets.progress, isLatest: data.historySets.isLatest, chunkOrder: data.historySets.chunkOrder, peerDataRequestSessionId: data.historySets.peerDataRequestSessionId };
  }
  ...
  if (messageUpsertList.length) {
    ...
    map["messages.upsert"] = { messages: messageUpsertList.map((m) => m.message), type };
  }
...
function concatChats(a, b) {
  if (b.unreadCount === null && a.unreadCount < 0) {
    a.unreadCount = void 0;
    b.unreadCount = void 0;
  }
