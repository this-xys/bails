...
const makeSocket = (config) => {
  ...
  const query = async (node, timeoutMs) => {
    ...
    const result = await promiseTimeout(timeoutMs, async (resolve, reject) => {
      const result2 = waitForMessage(msgId, timeoutMs).catch(reject);
      sendNode(node).then(async () => resolve(await result2)).catch(reject);
    });
  ...
  const executeUSyncQuery = async (usyncQuery) => {
    ...
    const userNodes = validUsers.map((user) => {
      return { tag: "user", attrs: { jid: !user.phone ? user.id : void 0 }, content: usyncQuery.protocols.map((a) => a.getUserElement(user)).filter((a) => a !== null) };
    });
    ...
    const iq = { tag: "iq", attrs: { to: S_WHATSAPP_NET, type: "get", xmlns: "usync" }, content: [{ tag: "usync", attrs: { context: usyncQuery.context, mode: usyncQuery.mode, sid: generateMessageTag(), last: "true", index: "0" }, content: [queryNode, listNode] }] };
  ...
  socketEndHandlers.push(() => keys.disposeTransactionStorage());
  ...
  const uploadPreKeys = async (count = MIN_PREKEY_COUNT) => {
    ...
    const uploadLogic = async (retryCount) => {
      ...
      const node = await keys.transaction(async () => {
        ...
        const { update, node: node2 } = await getNextPreKeysNode({ creds, keys }, count);
    ...
    uploadPreKeysPromise = Promise.race([uploadLogic(0), new Promise((_, reject) => setTimeout(() => reject(new Boom("Pre-key upload timeout", { statusCode: 408 })), UPLOAD_TIMEOUT))]);
    try {
      await uploadPreKeysPromise;
    } finally {
  ...
  const startKeepAliveRequest = () => keepAliveReq = setInterval(() => {
    ...
    if (diff > keepAliveIntervalMs + 5e3) {
      ...
      query({ tag: "iq", attrs: { id: generateMessageTag(), to: S_WHATSAPP_NET, type: "get", xmlns: "w:p" }, content: [{ tag: "ping", attrs: {} }] }).catch((err) => {
  ...
  const logout = async (msg) => {
    ...
    if (jid) {
      await sendNode({ tag: "iq", attrs: { to: S_WHATSAPP_NET, type: "set", id: generateMessageTag(), xmlns: "md" }, content: [{ tag: "remove-companion-device", attrs: { jid, reason: "user_initiated" } }] });
    }
  ...
  const sendWAMBuffer = (wamBuffer) => {
    return query({ tag: "iq", attrs: { to: S_WHATSAPP_NET, id: generateMessageTag(), xmlns: "w:stats" }, content: [{ tag: "add", attrs: { t: Math.round(Date.now() / 1e3) + "" }, content: wamBuffer }] });
  };
  ...
  ws.on("CB:iq,,pair-success", async (stanza) => {
    ...
    try {
      ...
      ev.emit("connection.update", { isNewLogin: true, qr: void 0 });
  ...
  process.nextTick(() => {
    ...
    ev.emit("connection.update", { connection: "connecting", receivedPendingNotifications: false, qr: void 0 });
  });
  ...
  const updateServerTimeOffset = ({ attrs }) => {
    ...
    serverTimeOffsetMs = parsed * 1e3 - localMs;
  ...
  const fetchAccountReachoutTimelock = async () => {
    ...
    const result = { isActive: !!queryResult?.is_active, timeEnforcementEnds: queryResult?.time_enforcement_ends && queryResult?.time_enforcement_ends !== "0" ? new Date(parseInt(queryResult.time_enforcement_ends, 10) * 1e3) : void 0, enforcementType: queryResult?.enforcement_type ?? ReachoutTimelockEnforcementType.DEFAULT };
  ...
  return { type: "md", ws, ev, authState: { creds, keys }, signalRepository, get user() {
    return authState.creds.me;
  }, generateMessageTag, query, waitForMessage, waitForSocketOpen, sendRawMessage, sendNode, logout, end, registerSocketEndHandler, onUnexpectedError, uploadPreKeys, uploadPreKeysToServerIfRequired, digestKeyBundle, rotateSignedPreKey, requestPairingCode, updateServerTimeOffset, sendUnifiedSession, wamBuffer: publicWAMBuffer, waitForConnectionUpdate: bindWaitForConnectionUpdate(ev), sendWAMBuffer, executeUSyncQuery, onWhatsApp, fetchAccountReachoutTimelock, fetchNewChatMessageCap };
};
