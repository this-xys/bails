...
import { delay, generateMessageID, generateWAMessage, generateWAMessageContent, generateWAMessageFromContent, getUrlFromDirectPath, normalizeMessageContent, prepareWAMessageMedia } from "../Utils/index.js";
...
class Dugong {
  ...
  detectType(content) {
    ...
    if (content.groupInviteMessage) return "GROUP_INVITE";
  ...
  async handlePayment(content, quoted) {
    ...
    return { requestPaymentMessage: proto.Message.RequestPaymentMessage.fromObject({ expiryTimestamp: data.expiry || 0, amount1000: data.amount || 0, currencyCodeIso4217: data.currency || "IDR", requestFrom: data.from || "0@s.whatsapp.net", noteMessage: notes, background: data.background ?? { id: "DEFAULT", placeholderArgb: 4293980400 } }) };
  }
  async handleProduct(content, _jid, _quoted) {
    ...
    return { viewOnceMessage: { message: { interactiveMessage: { body: { text: body }, footer: { text: footer }, header: { title, hasMediaAttachment: true, productMessage: { product: { productImage, productId, title, description, currencyCode, priceAmount1000, retailerId, url, productImageCount: 1 }, businessOwnerJid: "0@s.whatsapp.net" } }, nativeFlowMessage: { buttons } } } } };
  }
  async handleInteractive(content, _jid, _quoted) {
    ...
    const interactiveMessage = { body: { text: title || "" }, footer: { text: footer || "" } };
    ...
    if (media) {
      interactiveMessage.header = { title: header || "", hasMediaAttachment: true, ...media };
    } else {
      interactiveMessage.header = { title: header || "", hasMediaAttachment: false };
    }
    const finalContextInfo = {};
    if (contextInfo) {
      Object.assign(finalContextInfo, { mentionedJid: contextInfo.mentionedJid || [], forwardingScore: contextInfo.forwardingScore || 0, isForwarded: contextInfo.isForwarded || false, ...contextInfo });
    }
    if (externalAdReply) {
      finalContextInfo.externalAdReply = { title: externalAdReply.title || "", body: externalAdReply.body || "", mediaType: externalAdReply.mediaType || 1, thumbnailUrl: externalAdReply.thumbnailUrl || "", mediaUrl: externalAdReply.mediaUrl || "", sourceUrl: externalAdReply.sourceUrl || "", showAdAttribution: externalAdReply.showAdAttribution || false, renderLargerThumbnail: externalAdReply.renderLargerThumbnail || false, ...externalAdReply };
    }
  ...
  async handleInteractiveButtons(content, _jid, _quoted) {
    ...
    const buttons = interactiveButtons.map((btn) => ({ name: btn.name, buttonParamsJson: typeof btn.buttonParamsJson === "string" ? btn.buttonParamsJson : JSON.stringify(btn.buttonParamsJson) }));
    let headerContent = {};
    ...
    if (image) {
      ...
      if (jpegThumbnail) {
        uploaded.documentMessage.jpegThumbnail = typeof jpegThumbnail === "string" ? Buffer.from(jpegThumbnail, "base64") : jpegThumbnail;
      }
      headerContent = { ...uploaded };
      mediaAttached = typeof hasMediaAttachment === "boolean" ? hasMediaAttachment : true;
    } else if (location) {
      headerContent = { locationMessage: { degreesLatitude: location.degressLatitude || location.degreesLatitude || 0, degreesLongitude: location.degressLongitude || location.degreesLongitude || 0, name: location.name || "" } };
      mediaAttached = typeof hasMediaAttachment === "boolean" ? hasMediaAttachment : true;
    } else if (product) {
      let productImage;
    ...
    const interactiveMessage = { body: { text: bodyText }, footer: { text: footer || "" }, header: { title: title || "", subtitle: subtitle || "", hasMediaAttachment: mediaAttached, ...headerContent }, nativeFlowMessage: { buttons } };
    return { viewOnceMessage: { message: { messageContextInfo: { deviceListMetadata: {}, deviceListMetadataVersion: 2, messageSecret: crypto.randomBytes(32) }, interactiveMessage } } };
  }
  async handleCarousel(content, _jid, _quoted) {
    ...
    for (const card of carouselMessage.cards) {
      const cardMsg = { body: card.body || { text: "" }, footer: card.footer || { text: "" }, header: { title: card.header?.title || "", hasMediaAttachment: false } };
      ...
      if (card.header?.imageMessage || card.header?.videoMessage || card.header?.documentMessage) {
        ...
        cardMsg.header = { title: card.header?.title || "", hasMediaAttachment: mediaAttached, ...headerContent };
      }
      processedCards.push(cardMsg);
    }
    const interactiveMsg = { body: body || { text: "" }, footer: footer || { text: "" }, header: header || { title: "", hasMediaAttachment: false }, carouselMessage: { cards: processedCards, messageVersion: carouselMessage.messageVersion || 1, carouselCardType: carouselMessage.carouselCardType ?? 1 } };
    if (contextInfo) {
      interactiveMsg.contextInfo = contextInfo;
    }
    return { viewOnceMessage: { message: { messageContextInfo: { deviceListMetadata: {}, deviceListMetadataVersion: 2, messageSecret: crypto.randomBytes(32) }, interactiveMessage: interactiveMsg } } };
  }
  async handleAlbum(content, jid, quoted) {
    const array = content.albumMessage || content.album;
    const ctxInfo = content.contextInfo || {};
    const album = await generateWAMessageFromContent(jid, { messageContextInfo: { messageSecret: crypto.randomBytes(32) }, albumMessage: { expectedImageCount: array.filter((a) => "image" in a).length, expectedVideoCount: array.filter((a) => "video" in a).length } }, { userJid: jidNormalizedUser(this.sock.authState?.creds?.me?.id || ""), quoted, upload: this.waUploadToServer });
    ...
    for (let item of array) {
      ...
      try {
        const img = await generateWAMessage(jid, item, { upload: this.waUploadToServer, userJid: jidNormalizedUser(this.sock.authState?.creds?.me?.id || "") });
        img.message.messageContextInfo = { messageSecret: crypto.randomBytes(32), messageAssociation: { associationType: 1, parentMessageKey: album.key } };
  ...
  async handleEvent(content, jid, quoted) {
    ...
    const msg = await generateWAMessageFromContent(jid, { viewOnceMessage: { message: { messageContextInfo: { deviceListMetadata: {}, deviceListMetadataVersion: 2, messageSecret: crypto.randomBytes(32) }, eventMessage: { isCanceled: eventData.isCanceled || false, name: eventData.name, description: eventData.description, location: eventData.location || { degreesLatitude: 0, degreesLongitude: 0, name: "Location" }, joinLink: eventData.joinLink || "", startTime: typeof eventData.startTime === "string" ? parseInt(eventData.startTime) : eventData.startTime || Date.now(), endTime: typeof eventData.endTime === "string" ? parseInt(eventData.endTime) : eventData.endTime || Date.now() + 36e5, extraGuestsAllowed: eventData.extraGuestsAllowed !== false } } } }, { quoted, userJid: jidNormalizedUser(this.sock.authState?.creds?.me?.id || "") });
    await this.relayMessage(jid, msg.message, { messageId: msg.key.id });
    return msg;
  }
  async handlePollResult(content, jid, quoted) {
    const pollData = content.pollResultMessage;
    const msg = await generateWAMessageFromContent(jid, { pollResultSnapshotMessage: { name: pollData.name, pollVotes: pollData.pollVotes.map((vote) => ({ optionName: vote.optionName, optionVoteCount: typeof vote.optionVoteCount === "number" ? vote.optionVoteCount.toString() : vote.optionVoteCount })) } }, { quoted, userJid: jidNormalizedUser(this.sock.authState?.creds?.me?.id || "") });
    await this.relayMessage(jid, msg.message, { messageId: msg.key.id });
    return msg;
  }
  async handleGroupInvite(content, jid, quoted) {
    const inviteData = content.groupInviteMessage;
    const msg = await generateWAMessageFromContent(jid, { groupInviteMessage: { groupJid: inviteData.groupJid, inviteCode: inviteData.inviteCode, inviteExpiration: inviteData.inviteExpiration ?? 0, groupName: inviteData.groupName, jpegThumbnail: inviteData.jpegThumbnail, caption: inviteData.caption, groupType: inviteData.groupType } }, { quoted, userJid: jidNormalizedUser(this.sock.authState?.creds?.me?.id || "") });
    await this.relayMessage(jid, msg.message, { messageId: msg.key.id });
    return msg;
  }
  ...
  async sendStatusWhatsApp(content, jids = []) {
    ...
    const font = !isMedia ? content.font || Math.floor(Math.random() * 9) : void 0;
    const textColor = !isMedia ? content.textColor || getRandomHexColor() : void 0;
    const backgroundColor = !isMedia || isAudio ? content.backgroundColor || getRandomHexColor() : void 0;
    const ptt = isAudio ? typeof content.ptt === "boolean" ? content.ptt : true : void 0;
    const { getUrlInfo } = await import("../Utils/link-preview.js");
    const msg = await generateWAMessage(STORIES_JID, messageContent, { logger: this.config.logger, userJid, getUrlInfo: (text) => getUrlInfo(text, { thumbnailWidth: this.config.linkPreviewImageThumbnailWidth, fetchOpts: { timeout: 3e3, ...this.config.options || {} }, logger: this.config.logger, uploadImage: this.config.generateHighQualityLinkPreview ? this.waUploadToServer : void 0 }), upload: this.waUploadToServer, mediaCache: this.config.mediaCache, options: this.config.options, font, textColor, backgroundColor, ptt });
    await this.relayMessage(STORIES_JID, msg.message, { messageId: msg.key.id, statusJidList: uniqueUsers, additionalNodes: [{ tag: "meta", attrs: {}, content: [{ tag: "mentioned_users", attrs: {}, content: jids.map((jid) => ({ tag: "to", attrs: { jid: jidNormalizedUser(jid) } })) }] }] });
    for (const id of jids) {
      try {
        const normalizedId = jidNormalizedUser(id);
        const isPrivate = isPnUser(normalizedId);
        const type = isPrivate ? "statusMentionMessage" : "groupStatusMentionMessage";
        const protocolMessage = { [type]: { message: { protocolMessage: { key: msg.key, type: 25 } } }, messageContextInfo: { messageSecret: crypto.randomBytes(32) } };
        const statusMsg = await generateWAMessageFromContent(normalizedId, protocolMessage, { userJid: jidNormalizedUser(this.sock.authState?.creds?.me?.id || "") });
        await this.relayMessage(normalizedId, statusMsg.message, { additionalNodes: [{ tag: "meta", attrs: isPrivate ? { is_status_mention: "true" } : { is_group_status_mention: "true" } }] });
        await delay(2e3);
      } catch (error) {
