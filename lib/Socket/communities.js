...
const makeCommunitiesSocket = (config) => {
  ...
  const communityQuery = async (jid, type, content) => query({ tag: "iq", attrs: { type, xmlns: "w:g2", to: jid }, content });
  ...
  const communityFetchAllParticipating = async () => {
    const result = await query({ tag: "iq", attrs: { to: "@g.us", xmlns: "w:g2", type: "get" }, content: [{ tag: "participating", attrs: {}, content: [{ tag: "participants", attrs: {} }, { tag: "description", attrs: {} }] }] });
    const data = {};
  ...
  const Optional = { empty: () => null, of: (value) => value !== null ? { value } : null };
  ...
  return { ...sock, communityQuery, communityMetadata, communityCreate: async (subject, body) => {
    const descriptionId = generateMessageID().substring(0, 12);
    const result = await communityQuery("@g.us", "set", [{ tag: "create", attrs: { subject }, content: [{ tag: "description", attrs: { id: descriptionId }, content: [{ tag: "body", attrs: {}, content: Buffer.from(body || "", "utf-8") }] }, { tag: "parent", attrs: { default_membership_approval_mode: "request_required" } }, { tag: "allow_non_admin_sub_group_creation", attrs: {} }, { tag: "create_general_chat", attrs: {} }] }]);
    return await parseGroupResult(result);
  }, communityCreateGroup: async (subject, participants, parentCommunityJid) => {
    const key = generateMessageIDV2();
    const result = await communityQuery("@g.us", "set", [{ tag: "create", attrs: { subject, key }, content: [...participants.map((jid) => ({ tag: "participant", attrs: { jid } })), { tag: "linked_parent", attrs: { jid: parentCommunityJid } }] }]);
    return await parseGroupResult(result);
    ...
    await communityQuery(parentCommunityJid, "set", [{ tag: "links", attrs: {}, content: [{ tag: "link", attrs: { link_type: "sub_group" }, content: [{ tag: "group", attrs: { jid: groupJid } }] }] }]);
    ...
    if (subGroupsNode) {
      ...
      for (const groupNode of groupNodes) {
        linkedGroupsData.push({ id: groupNode.attrs.id ? jidEncode(groupNode.attrs.id, "g.us") : void 0, subject: groupNode.attrs.subject || "", creation: groupNode.attrs.creation ? Number(groupNode.attrs.creation) : void 0, owner: groupNode.attrs.creator ? jidNormalizedUser(groupNode.attrs.creator) : void 0, size: groupNode.attrs.size ? Number(groupNode.attrs.size) : void 0 });
      }
    }
    ...
    const result = await communityQuery(jid, "set", [{ tag: "membership_requests_action", attrs: {}, content: [{ tag: action, attrs: {}, content: participants.map((jid2) => ({ tag: "participant", attrs: { jid: jid2 } })) }] }]);
    ...
    const result = await communityQuery(jid, "set", [{ tag: action, attrs: action === "remove" ? { linked_groups: "true" } : {}, content: participants.map((jid2) => ({ tag: "participant", attrs: { jid: jid2 } })) }]);
    ...
    await communityQuery(jid, "set", [{ tag: "description", attrs: { ...description ? { id: generateMessageID() } : { delete: "true" }, ...prev ? { prev } : {} }, content: description ? [{ tag: "body", attrs: {}, content: Buffer.from(description, "utf-8") }] : void 0 }]);
    ...
    const results = await communityQuery(inviteMessage.groupJid, "set", [{ tag: "accept", attrs: { code: inviteMessage.inviteCode, expiration: inviteMessage.inviteExpiration.toString(), admin: key.remoteJid } }]);
    if (key.id) {
    ...
    await upsertMessage({ key: { remoteJid: inviteMessage.groupJid, id: generateMessageIDV2(sock.user?.id), fromMe: false, participant: key.remoteJid }, messageStubType: WAMessageStubType.GROUP_PARTICIPANT_ADD, messageStubParameters: [JSON.stringify(authState.creds.me)], participant: key.remoteJid, messageTimestamp: unixTimestampSeconds() }, "notify");
...
const extractCommunityMetadata = (result) => {
  ...
  const metadata = { id: communityId, subject: community.attrs.subject || "", subjectOwner: community.attrs.s_o, subjectTime: Number(community.attrs.s_t || 0), size: getBinaryNodeChildren(community, "participant").length, creation: Number(community.attrs.creation || 0), owner: community.attrs.creator ? jidNormalizedUser(community.attrs.creator) : void 0, desc, descId, linkedParent: getBinaryNodeChild(community, "linked_parent")?.attrs.jid || void 0, restrict: !!getBinaryNodeChild(community, "locked"), announce: !!getBinaryNodeChild(community, "announcement"), isCommunity: !!getBinaryNodeChild(community, "parent"), isCommunityAnnounce: !!getBinaryNodeChild(community, "default_sub_community"), joinApprovalMode: !!getBinaryNodeChild(community, "membership_approval_mode"), memberAddMode, participants: getBinaryNodeChildren(community, "participant").map(({ attrs }) => {
    return { id: attrs.jid, phoneNumber: isLidUser(attrs.jid) && isPnUser(attrs.phone_number) ? attrs.phone_number : void 0, lid: isPnUser(attrs.jid) && isLidUser(attrs.lid) ? attrs.lid : void 0, admin: attrs.type || null };
  }), ephemeralDuration: eph ? +eph : void 0, addressingMode: getBinaryNodeChildString(community, "addressing_mode") };
  return metadata;
};
export {
