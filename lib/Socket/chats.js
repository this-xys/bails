...
const makeChatsSocket = (config) => {
  ...
  let privacySettings;
  const serverProps = { privacyTokenOn1to1: true, profilePicPrivacyToken: true, lidTrustedTokenIssueToLid: false };
  ...
  const findUserId = async (pnLid) => {
    ...
    const userId = { lid: void 0, phoneNumber: void 0 };
  ...
  const getBusinessProfile = async (jid) => {
    ...
    if (profiles) {
      ...
      const businessHoursConfig = businessHours ? getBinaryNodeChildren(businessHours, "business_hours_config") : void 0;
      const websiteStr = website?.content?.toString();
      return { wid: profiles.attrs?.jid, address: address?.content?.toString(), description: description?.content?.toString() || "", website: websiteStr ? [websiteStr] : [], email: email?.content?.toString(), category: category?.content?.toString(), business_hours: { timezone: businessHours?.attrs?.timezone, business_config: businessHoursConfig?.map(({ attrs }) => attrs) } };
    }
  };
  ...
  const newAppStateChunkHandler = (isInitialSync) => {
    return { onMutation(mutation) {
      processSyncAction(mutation, ev, authState.creds.me, isInitialSync ? { accountSettings: authState.creds.accountSettings } : void 0, logger);
    } };
  };
  const resyncAppState = ev.createBufferedFunction(async (collections, isInitialSync) => {
    ...
    const getCachedAppStateSyncKey = async (keyId) => {
      if (appStateSyncKeyCache.has(keyId)) {
        return appStateSyncKeyCache.get(keyId) ?? void 0;
      }
    ...
    await authState.keys.transaction(async () => {
      ...
      while (collectionsToHandle.size) {
        ...
        for (const name of collectionsToHandle) {
          const result2 = await authState.keys.get("app-state-sync-version", [name]);
          let state = result2[name];
          if (state) {
  ...
  const profilePictureUrl = async (jid, type = "image", timeoutMs = 5e3, shouldIncludeTcToken = false) => {
    ...
    const result = await query({ tag: "iq", attrs: { target: jid, to: S_WHATSAPP_NET, type: "get", xmlns: "w:profile:picture" }, content }, timeoutMs);
    const child = getBinaryNodeChild(result, "picture");
    return child?.attrs?.url;
  };
  const createCallLink = async (type, event, timeoutMs) => {
    const result = await query({ tag: "call", attrs: { id: generateMessageTag(), to: "@call" }, content: [{ tag: "link_create", attrs: { media: type }, content: event ? [{ tag: "event", attrs: { start_time: String(event.startTime) } }] : void 0 }] }, timeoutMs);
  ...
  const presenceSubscribe = async (toJid) => {
    ...
    const tcTokenContent = isUserJid ? await buildTcTokenFromJid({ authState, jid: normalizedToJid, getLIDForPN }) : void 0;
  ...
  const handlePresenceUpdate = ({ tag, attrs, content }) => {
    ...
    if (tag === "presence") {
      presence = { lastKnownPresence: attrs.type === "unavailable" ? "unavailable" : "available", lastSeen: attrs.last && attrs.last !== "deny" ? +attrs.last : void 0, groupOnlineCount: attrs.count ? +attrs.count : void 0 };
  ...
  const waitForAppStateKeyId = (timeoutMs = 2e4, pollMs = 500) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      const timeout = setTimeout(() => {
        if (settled) return;
        cleanup();
        logger.warn({ hasMyAppStateKeyId: !!authState.creds.myAppStateKeyId, registered: authState.creds.registered }, "waitForAppStateKeyId timed out");
  ...
  const RESYNC_TTL_MS = 4e3;
  const lastResyncAt = /* @__PURE__ */ new Map();
  const appPatch = async (patchCreate) => {
    ...
    await appStatePatchMutex.mutex(async () => {
      await authState.keys.transaction(async () => {
        ...
        const node = { tag: "iq", attrs: { to: S_WHATSAPP_NET, type: "set", xmlns: "w:sync:app:state" }, content: [{ tag: "sync", attrs: {}, content: [{ tag: "collection", attrs: { name, version: (state.version - 1).toString(), return_snapshot: "false" }, content: [{ tag: "patch", attrs: {}, content: proto.SyncdPatch.encode(patch).finish() }] }] }] };
        await query(node);
    ...
    if (config.emitOwnEvents) {
      ...
      const { mutationMap } = await decodePatches(name, [{ ...encodeResult.patch, version: { version: encodeResult.state.version } }], initial, getAppStateSyncKey, config.options, void 0, logger);
  ...
  const fetchProps = async () => {
    ...
    if (privacyTokenProp !== void 0) {
      serverProps.privacyTokenOn1to1 = privacyTokenProp === "true" || privacyTokenProp === "1";
    }
    const profilePicProp = props["9666"] ?? props["profile_scraping_privacy_token_in_photo_iq"];
    if (profilePicProp !== void 0) {
      serverProps.profilePicPrivacyToken = profilePicProp === "true" || profilePicProp === "1";
    }
    const lidIssueProp = props["14303"] ?? props["lid_trusted_token_issue_to_lid"];
    if (lidIssueProp !== void 0) {
  ...
  const star = (jid, messages, star2) => {
    return chatModify({ star: { messages, star: star2 } }, jid);
  };
  ...
  const isWASystemNotification = (msg) => {
    ...
    if (protoType === proto.Message.ProtocolMessage.Type.HISTORY_SYNC_NOTIFICATION || protoType === proto.Message.ProtocolMessage.Type.APP_STATE_SYNC_KEY_SHARE || protoType === proto.Message.ProtocolMessage.Type.PEER_DATA_OPERATION_REQUEST_RESPONSE_MESSAGE || protoType === proto.Message.ProtocolMessage.Type.LID_MIGRATION_MAPPING_SYNC) {
      return true;
    }
    if (msg.messageStubType && !content.conversation && !content.extendedTextMessage) {
      const stub = msg.messageStubType;
      const SYSTEM_STUB_TYPES = new Set([WAMessageStubType.E2E_DEVICE_CHANGED, WAMessageStubType.E2E_ENCRYPTED, WAMessageStubType.E2E_IDENTITY_CHANGED, WAMessageStubType.CIPHERTEXT, WAMessageStubType.BIZ_PRIVACY_MODE_TO_BSP, WAMessageStubType.BIZ_PRIVACY_MODE_TO_FB]);
  ...
  const upsertMessage = ev.createBufferedFunction(async (msg, type) => {
    ...
    if (historyMsg && shouldProcessHistoryMsg) {
      ...
      if (syncType === proto.HistorySync.HistorySyncType.RECENT && historyMsg.progress === 100 && !historySyncStatus.recentSyncComplete) {
        ...
        historySyncPausedTimeout = void 0;
        ev.emit("messaging-history.status", { syncType, status: "complete", explicit: true });
      }
  ...
  return { ...sock, findUserId, serverProps, createCallLink, getBotListV2, messageMutex, receiptMutex, appStatePatchMutex, notificationMutex, fetchPrivacySettings, upsertMessage, appPatch, sendPresenceUpdate, presenceSubscribe, profilePictureUrl, fetchBlocklist, fetchStatus, fetchDisappearingDuration, updateProfilePicture, removeProfilePicture, updateProfileStatus, updateProfileName, updateBlockStatus, updateDisableLinkPreviewsPrivacy, updateCallPrivacy, updateMessagesPrivacy, updateLastSeenPrivacy, updateOnlinePrivacy, updateProfilePicturePrivacy, updateStatusPrivacy, updateReadReceiptsPrivacy, updateGroupsAddPrivacy, updateDefaultDisappearingMode, getBusinessProfile, resyncAppState, chatModify, cleanDirtyBits, addOrEditContact, removeContact, placeholderResendCache, addLabel, addChatLabel, removeChatLabel, addMessageLabel, removeMessageLabel, star, addOrEditQuickReply, removeQuickReply };
};
export {
  makeChatsSocket
};
