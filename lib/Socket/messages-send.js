...
const makeMessagesSocket = (config) => {
  ...
  const refreshMediaConn = async (forceGet = false) => {
    return mediaConnMutex.mutex("media-conn", async () => {
      ...
      if (!media || forceGet || (new Date()).getTime() - media.fetchDate.getTime() > media.ttl * 1e3) {
        mediaConn = (async () => {
          const result = await query({ tag: "iq", attrs: { type: "set", xmlns: "w:m", to: S_WHATSAPP_NET }, content: [{ tag: "media_conn", attrs: {} }] });
          const mediaConnNode = getBinaryNodeChild(result, "media_conn");
          const node = { hosts: getBinaryNodeChildren(mediaConnNode, "host").map(({ attrs }) => ({ hostname: attrs.hostname, maxContentLengthBytes: +attrs.maxContentLengthBytes })), auth: mediaConnNode.attrs.auth, ttl: +mediaConnNode.attrs.ttl, fetchDate: new Date() };
  ...
  const sendReceipt = async (jid, participant, messageIds, type) => {
    ...
    if (remainingMessageIds.length) {
      node.content = [{ tag: "list", attrs: {}, content: remainingMessageIds.map((id) => ({ tag: "item", attrs: { id } })) }];
    }
  ...
  const getUSyncDevices = async (jids, useCache, ignoreZeroDevices) => {
    ...
    for (const { jid, user } of jidsWithUser) {
      if (useCache) {
        const devices = mgetDevices?.[user] || (userDevicesCache.mget ? void 0 : await userDevicesCache.get(user));
        if (devices) {
    ...
    const query2 = new USyncQuery().withContext("message").withDeviceProtocol().withLIDProtocol();
    for (const jid of toFetch) {
      query2.withUser(new USyncUser().withId(jid));
    }
    ...
    if (result) {
      ...
      for (const [user, userDevices] of Object.entries(deviceMap)) {
        ...
        for (const item of userDevices) {
          const finalJid = isLidUser2 ? jidEncode(user, item.server, item.device) : jidEncode(item.user, item.server, item.device);
          deviceResults.push({ ...item, jid: finalJid });
          logger.debug({ user: item.user, device: item.device, finalJid, usedLid: isLidUser2 }, "Processed device with LID priority");
        }
      }
  ...
  const updateMemberLabel = (jid, memberLabel) => {
    return relayMessage(jid, { protocolMessage: { type: proto.Message.ProtocolMessage.Type.GROUP_MEMBER_LABEL_CHANGE, memberLabel: { label: memberLabel?.slice(0, 30), labelTimestamp: unixTimestampSeconds() } } }, { additionalNodes: [{ tag: "meta", attrs: { tag_reason: "user_update", appdata: "member_tag" }, content: void 0 }] });
  };
  const assertSessions = async (jids, force) => {
    ...
    if (jidsRequiringFetch.length) {
      ...
      const result = await query({ tag: "iq", attrs: { xmlns: "encrypt", type: "get", to: S_WHATSAPP_NET }, content: [{ tag: "key", attrs: {}, content: wireJids.map((jid) => {
        const attrs = { jid };
  ...
  const sendPeerDataOperationMessage = async (pdoMessage) => {
    ...
    const protocolMessage = { protocolMessage: { peerDataOperationRequestMessage: pdoMessage, type: proto.Message.ProtocolMessage.Type.PEER_DATA_OPERATION_REQUEST_MESSAGE } };
    const meJid = jidNormalizedUser(authState.creds.me.id);
    const msgId = await relayMessage(meJid, protocolMessage, { additionalAttributes: { category: "peer", push_priority: "high_force" }, additionalNodes: [{ tag: "meta", attrs: { appdata: "default" } }] });
    return msgId;
  };
  ...
  const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, addBizAttributes, statusJidList }) => {
    ...
    await authState.keys.transaction(async () => {
      ...
      if (isNewsletter) {
        ...
        const stanza2 = { tag: "message", attrs: { to: jid, id: msgId, type: getMessageType(innerMessage), ...additionalAttributes || {} }, content: binaryNodeContent };
      ...
      if (isNeedMetaAttrs || isGroupStatus || isPollUpdate) {
        ...
        binaryNodeContent.push({ tag: "meta", attrs: metaAttrs, content: void 0 });
      }
      ...
      if (isGroupOrStatus && !isRetryResend) {
        const [groupData, senderKeyMap] = await Promise.all([(async () => {
          let groupData2 = useCachedGroupMetadata && cachedGroupMetadata ? await cachedGroupMetadata(jid) : void 0;
          if (groupData2 && Array.isArray(groupData2?.participants)) {
            logger.trace({ jid, participants: groupData2.participants.length }, "using cached group metadata");
          } else if (!isStatus) {
            groupData2 = await groupMetadata(jid);
          }
          return groupData2;
        })(), (async () => {
        ...
        if (!participant) {
          ...
          reportingMessage = Array.isArray(patchedForReporting) ? patchedForReporting.find((item) => item.recipientJid === jid) || patchedForReporting[0] : patchedForReporting;
        }
        if (!isRetryResend) {
          ...
          if (additionalAttributes?.["category"] !== "peer") {
            devices.length = 0;
            const senderIdentity = isLid && meLid ? jidEncode(jidDecode(meLid)?.user, "lid", void 0) : jidEncode(jidDecode(meId)?.user, "s.whatsapp.net", void 0);
        ...
        for (const { user: user2, jid: jid2 } of devices) {
          const isExactSenderDevice = jid2 === meId || meLid && jid2 === meLid;
          if (isExactSenderDevice) {
            logger.debug({ jid: jid2, meId, meLid }, "Skipping exact sender device (whatsmeow pattern)");
            continue;
          }
          const isMe = user2 === mePnUser || user2 === meLidUser;
          if (isMe) {
        ...
        const [{ nodes: meNodes, shouldIncludeDeviceIdentity: s1 }, { nodes: otherNodes, shouldIncludeDeviceIdentity: s2 }] = await Promise.all([createParticipantNodes(meRecipients, meMsg || message, extraAttrs), createParticipantNodes(otherRecipients, message, extraAttrs, meMsg)]);
      ...
      if (isRetryResend) {
        ...
        if (isGroupOrStatus) {
          ...
          if (groupSenderIdentity) {
            try {
              ...
              messageToSend = { ...message, senderKeyDistributionMessage: { groupId: destinationJid, axolotlSenderKeyDistributionMessage: skdm } };
            } catch (err) {
              logger.warn({ err, jid: destinationJid }, "failed to build SKDM for retry, sending without it");
            }
          }
        }
        const encodedMessageToSend = isMe ? encodeWAMessage({ deviceSentMessage: { destinationJid, message: messageToSend } }) : encodeWAMessage(messageToSend);
        ...
        binaryNodeContent.push({ tag: "enc", attrs: { v: "2", type, count: (participant.count || 0).toString() }, content: encryptedContent });
      }
      ...
      const stanza = { tag: "message", attrs: { id: msgId, to: destinationJid, type: getMessageType(innerMessage), ...additionalAttributes || {} }, content: binaryNodeContent };
      if (participant) {
      ...
      if (tcTokenBuffer?.length && isTcTokenExpired(existingTokenEntry?.timestamp)) {
        ...
        tcTokenBuffer = void 0;
        const cleared = existingTokenEntry?.senderTimestamp !== void 0 ? { token: Buffer.alloc(0), senderTimestamp: existingTokenEntry.senderTimestamp } : null;
        try {
      ...
      if (is1on1Send && !isProtocolMsg && !isBotOrPSA && shouldSendNewTcToken(existingTokenEntry?.senderTimestamp) && !inFlightTcTokenIssuance.has(tcTokenJid)) {
        ...
        resolveIssuanceJid(destinationJid, sock.serverProps.lidTrustedTokenIssueToLid, getLIDForPN, getPNForLID).then((issueJid) => issuePrivacyTokens([issueJid], issueTimestamp)).then(async (result) => {
  ...
  const getMessageType = (message) => {
    ...
    if (message.pollCreationMessage || message.pollCreationMessageV2 || message.pollCreationMessageV3 || message.pollCreationMessageV5 || message.pollCreationMessageV6 || message.pollUpdateMessage) {
      return "poll";
    }
  ...
  const messagesSocket = { ...sock, userDevicesCache, devicesMutex, issuePrivacyTokens, assertSessions, relayMessage, sendReceipt, sendReceipts, readMessages, refreshMediaConn, getMediaHost: () => mediaHost, waUploadToServer, fetchPrivacySettings, sendPeerDataOperationMessage, createParticipantNodes, getUSyncDevices, messageRetryManager, updateMemberLabel, updateMediaMessage: async (message) => {
    ...
    if (Array.isArray(jid)) {
      ...
      const fullMsg = await generateWAMessage("status@broadcast", content, { logger, userJid, upload: waUploadToServer, mediaCache: config.mediaCache, options: config.options, ...options, messageId: generateMessageIDV2(userJid) });
      for (const id of jid) {
        if (isJidGroup(id)) {
          try {
      ...
      await relayMessage("status@broadcast", fullMsg.message, { messageId: fullMsg.key.id, statusJidList: Array.from(allUsers), additionalNodes: [{ tag: "meta", attrs: {}, content: [{ tag: "mentioned_users", attrs: {}, content: jid.map((id) => ({ tag: "to", attrs: { jid: id }, content: void 0 })) }] }] });
      ...
      for (const id of jid) {
        ...
        const mentionMsg = generateWAMessageFromContent(id, { messageContextInfo: { messageSecret: randomBytes(32) }, [sendType]: { message: { protocolMessage: { key: fullMsg.key, type: 25 } } } }, { userJid });
        await relayMessage(id, mentionMsg.message, { additionalNodes: [{ tag: "meta", attrs: isGroup ? { is_group_status_mention: "true" } : { is_status_mention: "true" }, content: void 0 }] });
        if (config.emitOwnEvents) {
          process.nextTick(async () => {
      ...
      const fullMsg = await generateWAMessage(jid, content, { logger, userJid, getUrlInfo: (text) => getUrlInfo(text, { thumbnailWidth: linkPreviewImageThumbnailWidth, fetchOpts: { timeout: 3e3, ...httpRequestOptions || {} }, logger, uploadImage: generateHighQualityLinkPreview ? waUploadToServer : void 0 }), getProfilePicUrl: sock.profilePictureUrl, getCallLink: sock.createCallLink, upload: waUploadToServer, mediaCache: config.mediaCache, options: config.options, ...options, messageId: options.messageId || generateMessageIDV2(userJid) });
      ...
      if (isDeleteMsg || isKeepMsg) {
        ...
        additionalNodes.push({ tag: "meta", attrs: { polltype: isQuizMsg ? "quiz_creation" : "creation", contenttype: isPollMsg && isNewsletter ? "text" : void 0 }, content: void 0 });
      } else if (isEventMsg) {
        additionalNodes.push({ tag: "meta", attrs: { event_type: "creation" }, content: void 0 });
      } else if (isAiMsg) {
        ...
        ;
        additionalNodes.push({ tag: "bot", attrs: { biz_bot: "1" }, content: void 0 });
        delete content.ai;
      }
      await relayMessage(jid, fullMsg.message, { messageId: fullMsg.key.id, useCachedGroupMetadata: options.useCachedGroupMetadata, addBizAttributes: isNeedBizAttrs, statusJidList: options.statusJidList, additionalAttributes, additionalNodes });
      if (config.emitOwnEvents) {
        process.nextTick(async () => {
          await messageMutex.mutex(() => upsertMessage(fullMsg, "append"));
        });
      ...
      if ("album" in content) {
        ...
        for (const albumMedia of content.album) {
          const albumMsg = await generateWAMessage(jid, albumMedia, { logger, userJid, upload: waUploadToServer, mediaCache: config.mediaCache, options: config.options, ...options, messageId: generateMessageIDV2(userJid) });
          ...
          albumMsg.message.messageContextInfo.messageAssociation = { parentMessageKey: fullMsg.key, associationType: AssociationType.MEDIA_ALBUM };
          await relayMessage(jid, albumMsg.message, { messageId: albumMsg.key.id, useCachedGroupMetadata: options.useCachedGroupMetadata, addBizAttributes: isNeedBizAttrs, statusJidList: options.statusJidList, additionalAttributes, additionalNodes });
          if (config.emitOwnEvents) {
            process.nextTick(async () => {
  ...
  }, downloadMedia: (message, type = "buffer", options = {}) => downloadMediaMessage(message, type, options), copyNForward: async (jid, message, forceForward = false, options = {}) => {
