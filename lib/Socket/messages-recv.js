...
const makeMessagesRecvSocket = (config) => {
  ...
  const handleReachoutTimelockNotification = (data) => {
    ...
    if (!payload.is_active) {
      ...
      ev.emit("connection.update", { reachoutTimeLock: { isActive: false, enforcementType: ReachoutTimelockEnforcementType.DEFAULT } });
      return;
    }
    const timeEnforcementEnds = payload.time_enforcement_ends ? new Date(parseInt(payload.time_enforcement_ends, 10) * 1e3) : new Date(Date.now() + 6e4);
    const enforcementType = isValidEnforcementType(payload.enforcement_type) ? payload.enforcement_type : ReachoutTimelockEnforcementType.DEFAULT;
    logger.info({ enforcementType, timeEnforcementEnds }, "reachout timelock restriction set");
    ev.emit("connection.update", { reachoutTimeLock: { isActive: true, timeEnforcementEnds, enforcementType } });
  };
      ...
      case "NotificationNewsletterUpdate":
        for (const update of updates) {
          if (update.jid && update.settings && Object.keys(update.settings).length > 0) {
            ev.emit("newsletter-settings.update", { id: update.jid, update: update.settings });
          }
        }
        break;
      case "NotificationNewsletterAdminPromote":
        for (const update of updates) {
          if (update.jid && update.user) {
            ev.emit("newsletter-participants.update", { id: update.jid, author: node.attrs.from, user: update.user, new_role: "ADMIN", action: "promote" });
          }
        }
        break;
      case "NotificationLinkedProfilesUpdates":
  ...
  const handleNewsletterNotification = async (node) => {
    ...
    for (const child of children) {
      ...
      switch (child.tag) {
        case "reaction": {
          const reactionUpdate = { id: from, server_id: child.attrs.message_id, reaction: { code: getBinaryNodeChildString(child, "reaction"), count: 1 } };
          ev.emit("newsletter.reaction", reactionUpdate);
          break;
        }
        case "view": {
          const viewUpdate = { id: from, server_id: child.attrs.message_id, count: parseInt(child.content?.toString() || "0", 10) };
          ev.emit("newsletter.view", viewUpdate);
          break;
        }
        case "participant": {
          const participantUpdate = { id: from, author, user: child.attrs.jid, action: child.attrs.action, new_role: child.attrs.role };
  ...
  const rejectCall = async (callId, callFrom) => {
    const stanza = { tag: "call", attrs: { from: authState.creds.me.id, to: callFrom }, content: [{ tag: "reject", attrs: { "call-id": callId, "call-creator": callFrom, count: "0" }, content: void 0 }] };
    await query(stanza);
  };
  const sendRetryRequest = async (node, forceIncludeKeys = false) => {
    ...
    if (messageRetryManager) {
      ...
      if (retryCount2 >= maxMsgRetryCount) {
        logger.debug({ retryCount: retryCount2, msgId }, "reached retry limit, clearing");
    ...
    if (retryCount <= 2) {
      if (messageRetryManager) {
        ...
        logger.debug(`sendRetryRequest: requested placeholder resend for message ${msgId2}`);
      }
    }
    ...
    await authState.keys.transaction(async () => {
      ...
      if (retryCount > 1 || forceIncludeKeys || shouldRecreateSession) {
        ...
        const key2 = preKeys[+keyId];
        const content = receipt.content;
        content.push({ tag: "keys", attrs: {}, content: [{ tag: "type", attrs: {}, content: Buffer.from(KEY_BUNDLE_TYPE) }, { tag: "identity", attrs: {}, content: identityKey.public }, xmppPreKey(key2, +keyId), xmppSignedPreKey(signedPreKey), { tag: "device-identity", attrs: {}, content: deviceIdentity }] });
  ...
  const reissueTcTokenAfterIdentityChange = (from) => {
    void (async () => {
      ...
      if (senderTs === null || senderTs === void 0 || isTcTokenExpired(senderTs)) {
        return;
      }
  ...
  const handleGroupNotification = (fullNode, child, msg) => {
    ...
    switch (child?.tag) {
      case "create":
        ...
        ev.emit("groups.upsert", [{ ...metadata, author: actingParticipantLid, authorPn: actingParticipantPn, authorUsername: actingParticipantUsername }]);
        break;
      case "ephemeral":
      case "not_ephemeral":
      ...
      case "leave":
        ...
        const participants = getBinaryNodeChildren(child, "participant").map(({ attrs }) => {
          return { id: attrs.jid, phoneNumber: isLidUser(attrs.jid) && isPnUser(attrs.phone_number) ? attrs.phone_number : void 0, lid: isPnUser(attrs.jid) && isLidUser(attrs.lid) ? attrs.lid : void 0, username: attrs.participant_username || attrs.username || void 0, admin: attrs.type || null };
        });
        if (participants.length === 1 && (areJidsSameUser(participants[0].id, actingParticipantLid) || areJidsSameUser(participants[0].id, actingParticipantPn)) && child.tag === "remove") {
  ...
  const processNotification = async (node) => {
    ...
    switch (nodeType) {
      ...
      case "picture":
        ...
        ev.emit("contacts.update", [{ id: contactJid || "", imgUrl: setPicture ? "changed" : "removed" }]);
        if (isJidGroup(from)) {
          ...
          result.key = { ...result.key || {}, participant: pictureNode?.attrs.author };
        }
        break;
      case "account_sync":
  ...
  const handlePrivacyTokenNotification = async (node) => {
    ...
    const from = jidNormalizedUser(node.attrs.from);
    const senderLid = node.attrs.sender_lid && isLidUser(jidNormalizedUser(node.attrs.sender_lid)) ? jidNormalizedUser(node.attrs.sender_lid) : void 0;
      ...
      await Promise.all([receiptMutex.mutex(async () => {
        ...
        if (typeof status !== "undefined" && (status >= proto.WebMessageInfo.Status.SERVER_ACK || !isNodeFromMe)) {
          if (isJidGroup(remoteJid) || isJidStatusBroadcast(remoteJid)) {
            ...
            ev.emit("messages.update", ids.map((id) => ({ key: { ...key, id }, update: { status, messageTimestamp: toNumber(+(attrs.t ?? 0)) } })));
          }
        }
        if (attrs.type === "retry") {
    ...
    try {
      await Promise.all([notificationMutex.mutex(async () => {
        ...
        if (msg) {
          ...
          msg.key = { remoteJid, fromMe, participant: node.attrs.participant, participantAlt, participantUsername: node.attrs.participant_username, addressingMode, id: node.attrs.id, ...msg.key || {} };
  ...
  const handleMessage = async (node) => {
    ...
    if (encNode?.attrs.type === "msmsg") {
      ...
      await sendMessageAck(node, NACK_REASONS.MissingMessageSecret).catch((ackErr) => logger.error({ ackErr }, "failed to ack msmsg"));
      return;
    }
    let acked = false;
    try {
          ...
          if (msg.messageStubParameters?.[0] === NO_MESSAGE_FOUND_ERROR_TEXT) {
            ...
            requestPlaceholderResend(cleanKey, msgData).then((requestId) => {
              if (requestId && requestId !== "RESOLVED") {
                ...
                ev.emit("messages.update", [{ key: msg.key, update: { messageStubParameters: [NO_MESSAGE_FOUND_ERROR_TEXT, requestId] } }]);
              }
            }).catch((err) => {
  ...
  const handleCall = async (node) => {
    try {
      ...
      const call = { chatId: attrs.from, from, callerPn: infoChild.attrs["caller_pn"], id: callId, date: new Date(+attrs.t * 1e3), offline: !!attrs.offline, status };
      if (status === "relaylatency") {
        const latencyValue = infoChild.attrs.latency || infoChild.attrs["latency_ms"] || infoChild.attrs["latency-ms"];
        const latencyMs = latencyValue ? Number(latencyValue) : void 0;
  ...
  const handleBadAck = async ({ attrs }) => {
    ...
    if (attrs.phash && config.phashRetryEnabled) {
      ...
      if (msg) {
        await delay(100);
        await relayMessage(key.remoteJid, msg, { messageId: key.id, useUserDevicesCache: false, phashRetry: true }).catch((err) => {
  ...
  const offlineNodeProcessor = makeOfflineNodeProcessor(new Map([["message", handleMessage], ["call", handleCall], ["receipt", handleReceipt], ["notification", handleNotification]]), { isWsOpen: () => ws.isOpen, onUnexpectedError, yieldToEventLoop: () => new Promise((resolve) => setImmediate(resolve)) });
  const processNode = async (type, node, identifier, exec) => {
    ...
    if (ignoreJid && ignoreJid !== S_WHATSAPP_NET && shouldIgnoreJid(ignoreJid)) {
      await sendMessageAck(node, type === "message" ? NACK_REASONS.UnhandledError : void 0).catch((ackErr) => logger.error({ ackErr }, "failed to ack ignored-jid node"));
      return;
    }
  ...
  async function pruneExpiredTcTokens() {
    try {
      ...
      for (const jid of jids) {
        ...
        const hasSenderTs = entry.senderTimestamp !== void 0;
