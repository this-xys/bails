...
class LIDMappingStore {
  constructor(keys, logger, pnToLIDFunc) {
    this.mappingCache = new LRUCache({ max: 10000, ttl: 3 * 24 * 60 * 60 * 1e3, ttlAutopurge: false, updateAgeOnGet: true });
  ...
  async _getLIDsForPNsImpl(pns) {
    ...
    const addResolvedPair = (pn, decoded, lidUser) => {
      ...
      const pnDevice = decoded.device !== void 0 ? decoded.device : 0;
  ...
  async _getPNsForLIDsImpl(lids) {
    ...
    const addResolvedPair = (lid, decoded, pnUser) => {
      ...
      const lidDevice = decoded.device !== void 0 ? decoded.device : 0;
