...
export class AudioFeeder {
  ...
  start=() => {
    ...
    const chunkIntervalMs = this.framesPerChunk / this.sampleRate * 1e3;
    const inputArgs = this.#resolveInputArgs();
    this.#proc = spawn("ffmpeg", [ "-hide_banner", "-loglevel", "error", "-thread_queue_size", "512", ...inputArgs, "-f", "f32le", "-ac", String(this.channels), "-ar", String(this.sampleRate), "pipe:1" ]);
