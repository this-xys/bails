...
const ICE_RESTART_IDLE_THRESHOLD_MS = 1e4;

const ICE_RTT_POLL_MS = 1e3;
...
const classifyRelayPacket = packet => {
  ...
  if ((first & 192) !== 0) return "non_stun";
  const stunType = (first & 63) << 8 | second;
  if (stunType === 1) return "stun_bind";
  if (stunType === 3) return "stun_alloc";
  return "stun_unknown";
};

const appendRelayPacketLog = (direction, connection, packet) => {
  if (!RELAY_PACKET_LOG_PATH) return;
  const buffer = packet instanceof ArrayBuffer ? Buffer.from(new Uint8Array(packet)) : Buffer.from(packet.buffer, packet.byteOffset, packet.byteLength);
...
export class RelayRtcTransport {
  ...
  updateRelayList=update => {
    ...
    for (const relay of update.relays ?? []) {
      ...
      const authToken = update.auth_tokens && relay.auth_token_id != null && relay.auth_token_id >= 0 ? update.auth_tokens[relay.auth_token_id] : undefined;
  ...
  send=(packet, ip, port) => {
    ...
    const candidateIds = [ requestedId, getConnectionIdentifier(ip, preferredPort), getRelayLookupId(ip, TRUE_WEB_CLIENT_RELAY_PORT), getRelayLookupId(ip, FAUX_WEB_CLIENT_RELAY_PORT) ];
    let info;
    ...
    if (classifyRelayPacket(arrayBuffer) === "stun_alloc" && connection.state === "open" && connection.sentMedia && Date.now() - connection.lastRxPacketTime > ICE_RESTART_IDLE_THRESHOLD_MS) {
  ...
  #connect=async connection => {
    ...
    const {RTCPeerConnection: RTCPeerConnection} = wrtcModule;
  ...
  #restartIce=async connection => {
    ...
    const {RTCPeerConnection: RTCPeerConnection} = await this.#loadWrtc();
  ...
  #flushBufferedPackets=connection => {
    while (connection.state === "open" && connection.dataChannel?.readyState === "open" && connection.packetBuffer.length > 0) {
  ...
  #readCurrentRoundTripTimeMs=async pc => {
    try {
      ...
      const reports = stats && typeof stats.values === "function" ? Array.from(stats.values()) : Array.isArray(stats) ? stats : Object.values(stats ?? {});
      ...
      const selectedPair = selectedCandidatePairId && candidatePairs.find(r => r?.id === selectedCandidatePairId) || candidatePairs.find(r => r?.selected || r?.nominated) || candidatePairs[0];
      const rttSeconds = typeof selectedPair?.currentRoundTripTime === "number" ? selectedPair.currentRoundTripTime : typeof selectedPair?.totalRoundTripTime === "number" && typeof selectedPair?.responsesReceived === "number" && selectedPair.responsesReceived > 0 ? selectedPair.totalRoundTripTime / selectedPair.responsesReceived : null;
      if (rttSeconds == null || !Number.isFinite(rttSeconds) || rttSeconds <= 0) return null;
      return Math.max(1, Math.round(rttSeconds * 1e3));
    } catch {
      return null;
    }
  };
