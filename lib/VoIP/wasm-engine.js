...
const VOIP_READY_TIMEOUT_MS = 15e3;
...
class NodeWorkerMessagePort {
  ...
  postMessage=(msg, transferList) => {
    const out = msg && typeof msg === "object" && msg.cmd && !msg.type ? {
      ...msg,
      type: "cmd"
    } : msg;
  ...
  removeAllMessageListeners=type => {
    if (type) this.#listeners.get(type)?.clear(); else this.#listeners.clear();
  };
  terminate=() => {
  ...
  #handleMessage=data => {
    ...
    if (data.type === "callback" || data.type === "waWasmWorkerCompatibleCallback") {
      ...
      if (!callbackArgs || Object.keys(callbackArgs).length === 0 || Object.keys(callbackArgs).length === 1 && callbackArgs.prototype) {
        listenerData = {};
        for (const key in data) {
          if (key !== "type" && key !== "__name" && key !== "prototype" && key !== "args" && !key.startsWith("__")) {
...
export class WasmEngine {
  ...
  #loaderCode="";
  constructor(config = {}) {
    const basePath = config.resourcesPath ? path.isAbsolute(config.resourcesPath) ? config.resourcesPath : path.resolve(process.cwd(), config.resourcesPath) : path.resolve(__dirname, "..");
    const wasmPath = config.wasmPath ? path.isAbsolute(config.wasmPath) ? config.wasmPath : path.resolve(process.cwd(), config.wasmPath) : path.join(basePath, "assets", "wasm", "whatsapp.wasm");
    this.#config = {
      ...config,
      wasmPath: wasmPath,
  ...
  initialize=async () => {
    ...
    try {
      if (!fs.existsSync(voipStorageDir)) fs.mkdirSync(voipStorageDir, {
        recursive: true
      });
    } catch {}
    ...
    const wasmBuffer = this.#config.wasmBinary ? Buffer.from(this.#config.wasmBinary) : fs.readFileSync(this.#config.wasmPath);
    ...
    this.#loaderCode = this.#config.loaderCode ?? (workerBundleHasLoader ? "" : fs.existsSync(loaderFile) ? fs.readFileSync(loaderFile, "utf8") : "");
    ...
    const loaderModuleNames = [ this.#config.loaderModuleName, "WAWebVoipWebWasmLoader", "WAWebVoipWebWasmLoader.worker", "WAWebVoipWebWasmLoader_ProdLab_internal.worker", "WAWebVoipWebWasmLoader_ProdLabvideo_internal.worker" ].filter((v, i, a) => !!v && a.indexOf(v) === i);
    let wasmLoader = null;
    ...
    if (!_a.#globalCallbacksRegistered) this.#registerGlobalCallbacks();
    ...
    this.#voipStackInitPromise = new Promise(resolveInit => {
      ...
      try {
        ...
        try {
          ...
          if (modernErr?.name === "BindingError" && (String(modernErr?.message ?? "").includes("expected 8 args") || String(modernErr?.message ?? "").includes("takes 8"))) {
            this.#instance.initVoipStack(selfJid, meUserJid, selfLid, true, 5, 0, 8, 16);
          } else {
            throw modernErr;
          }
        }
        Promise.race([ this.#voipReadyPromise, new Promise(r => setTimeout(() => {
          this.#voipStackInitialized = true;
          r();
        }, VOIP_READY_TIMEOUT_MS)) ]).finally(() => {
  ...
  #startAudioPlaybackLoop=() => {
    ...
    this.#audioPlaybackLoopInterval = setInterval(() => {
      ...
      try {
        ...
        const hasNonZero = audioData.some(s => Math.abs(s) > 1e-4);
  ...
  #applyDefaultAbProps=() => {
    ...
    const setInt = typeof this.#instance.setABPropInt === "function" ? (k, v) => {
      this.#instance.setABPropInt(k, v);
    } : null;
    const setBool = typeof this.#instance.setABPropBool === "function" ? (k, v) => {
      this.#instance.setABPropBool(k, v);
    } : null;
    const setString = typeof this.#instance.setABPropString === "function" ? (k, v) => {
    ...
    for (const [key, value] of Object.entries(intProps)) {
      if (setInt && Number.isFinite(value)) try {
        setInt(key, value);
      } catch {}
    }
    ...
    for (const [key, value] of Object.entries(overrideProps)) {
      try {
        if (typeof value === "boolean" && setBool) setBool(key, value); else if (typeof value === "number" && setInt) setInt(key, value); else if (typeof value === "string" && setString) setString(key, value);
      } catch {}
    }
  };
  ...
  #registerGlobalCallbacks=() => {
    ...
    if (callbacks.onSignalingXmpp) {
      _a.registerGlobalCallbackListener("onSignalingXmpp", data => {
        ...
        if (Array.isArray(xmlPayload)) xmlPayload = new Uint8Array(xmlPayload); else if (xmlPayload && typeof xmlPayload === "object" && !(xmlPayload instanceof Uint8Array) && !Buffer.isBuffer(xmlPayload)) {
    ...
    if (callbacks.sendDataToRelay) {
      _a.registerGlobalCallbackListener("sendDataToRelay", data => {
        ...
        if (relayData instanceof Uint8Array) {} else if (Array.isArray(relayData)) relayData = new Uint8Array(relayData); else if (Buffer.isBuffer(relayData)) relayData = new Uint8Array(relayData); else if (relayData && typeof relayData === "object" && relayData.buffer) {
          relayData = new Uint8Array(relayData.buffer, relayData.byteOffset ?? 0, relayData.byteLength ?? relayData.length);
        } else if (relayData instanceof ArrayBuffer) relayData = new Uint8Array(relayData); else return 0;
    ...
    const preDefinedModules = {
      ...
      WorkerBundleResource: {
        createDedicatedWebWorker: resource => {
          const scriptPath = resource?.resourcePath && fs.existsSync(resource.resourcePath) ? resource.resourcePath : resolveWorkerScriptPath();
      ...
      bx: Object.assign(id => String(id), {
        getURL: () => ""
      }),
      HasteSupportData: {
        handle: () => {}
      },
      ServiceWorkerDynamicModules: {
        handle: () => {}
      },
      WhatsAppWebServiceWorker: {
        default: true
      },
      WAWebLogger: {
        initializeWAWebLogger: () => {}
      },
      WAWebSw: {
        initHandlers: () => {}
      },
  ...
  #createVMContext=memory => {
    ...
    const context = vm.createContext({
      ...
      addRunDependency: addRunDependency,
      removeRunDependency: removeRunDependency,
  ...
  #createAtomicsWrapper=_memory => {
    const atomicsWrapper = {
      ...
      waitAsync: Atomics.waitAsync ? Atomics.waitAsync.bind(Atomics) : () => ({
        async: true,
