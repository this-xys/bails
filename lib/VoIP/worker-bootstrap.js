...
global.babelHelpers = {
  ...
  wrapNativeSuper: function(Class) {
    ...
    function _getPrototypeOf(o) {
      return Object.getPrototypeOf || function(o) {
        return o.__proto__;
      };
    }
...
bxFunc.getURL = function(_id, _opts) {
  return "";
};
...
__modules["WorkerMessagePort"] = {
  exports: {
    WorkerSyncedMessagePort: function() {}
  }
};

__modules["WAWebVoipWebWasmWorkerResource"] = {
  exports: {}
};
...
class WorkerSyncedMessagePort {
  ...
  postMessage(data, transfer) {
    ...
    if (parentPort) {
      if (transfer) parentPort.postMessage(data, transfer); else parentPort.postMessage(data);
    }
  }
...
let WACryptoHkdfSync = {
  hkdf: function(key, salt, info, length) {
    const prk = crypto.createHmac("sha256", salt || Buffer.alloc(32)).update(key).digest();
    const n = Math.ceil(length / 32);
    const okm = Buffer.alloc(n * 32);
    let prev = Buffer.alloc(0);
    for (let i = 0; i < n; i++) {
      prev = crypto.createHmac("sha256", prk).update(Buffer.concat([ prev, info || Buffer.alloc(0), Buffer.from([ i + 1 ]) ])).digest();
      prev.copy(okm, i * 32);
    }
...
const WACryptoSha256HmacBuilder = {
  Sha256HMacBuilder: Sha256HMacBuilder
};
...
const getPreferredLoaderModuleNames = () => [ typedWorkerData?.loaderModuleName, "WAWebVoipWebWasmLoader", "WAWebVoipWebWasmLoader.worker", "WAWebVoipWebWasmLoader_ProdLab_internal.worker", "WAWebVoipWebWasmLoader_ProdLabvideo_internal.worker" ].filter((value, index, array) => !!value && array.indexOf(value) === index);
      ...
      return new Promise((resolve, reject) => {
        function step(key, arg) {
          try {
            ...
            if (info.done) resolve(value); else Promise.resolve(value).then(val => step("next", val), err => step("throw", err));
          } catch (e) {
            reject(e);
          }
        }
...
const getActiveWasmModule = () => _ || global.self.__waModule || s;
...
const handleRawVideoFrameOnJsWorker = msg => {
  ...
  if (!moduleRef || typeof moduleRef._malloc !== "function" || typeof moduleRef._free !== "function" || typeof sendFrameFn !== "function" || !(frameBuffer instanceof ArrayBuffer || ArrayBuffer.isView(frameBuffer))) {
    return;
  }
  const bytes = ArrayBuffer.isView(frameBuffer) ? new Uint8Array(frameBuffer.buffer, frameBuffer.byteOffset, frameBuffer.byteLength) : new Uint8Array(frameBuffer);
